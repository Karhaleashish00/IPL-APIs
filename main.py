...
db = client['IPL']  # Replace <dbname> with your database name
# MongoDB configuration
# app.config["MONGO_URI"] = "mongodb://localhost:27017/IPL"
# mongo = PyMongo(app)
...
matchs_collection = db.IPLMatches
stats_collection = db.IPLStatistics
...
seassion2008_collection = db.seassion2024
...
def get_player(player_name,year):
    ...
    team_cursor = db.players.find({'player_name': player_name, 'year': year}, {'team': 1, '_id': 0})
...
def get_csk_players():
    ...
    csk_players = db.images.find_one({}, {"IPL 2024.Chennai Super Kings": 1, "_id": 0})
...
def get_dc_players():
    # Find the document where IPL2024's key is "Chennai Super Kings"
    dc_players = db.images.find_one({}, {"IPL 2024.Delhi Capitals": 1, "_id": 0})
...
def get_gt_players():
    # Find the document where IPL2024's key is "Chennai Super Kings"
    gt_players = db.images.find_one({}, {"IPL 2024.Gujarat Titans": 1, "_id": 0})
...
def get_kkr_players():
    # Find the document where IPL2024's key is "Chennai Super Kings"
    kkr_players = db.images.find_one({}, {"IPL 2024.Kolkata Knight Riders": 1, "_id": 0})
...
def get_lsg_players():
    # Find the document where IPL2024's key is "Chennai Super Kings"
    lsg_players = db.images.find_one({}, {"IPL 2024.Lucknow Super Giants": 1, "_id": 0})
...
def get_mi_players():
    # Find the document where IPL2024's key is "Chennai Super Kings"
    mi_players = db.images.find_one({}, {"IPL 2024.Mumbai Indians": 1, "_id": 0})
...
def get_pbks_players():
    # Find the document where IPL2024's key is "Chennai Super Kings"
    pbks_players = db.images.find_one({}, {"IPL 2024.Punjab Kings": 1, "_id": 0})
...
def get_rr_players():
    # Find the document where IPL2024's key is "Chennai Super Kings"
    rr_players = db.images.find_one({}, {"IPL 2024.Rajasthan Royals": 1, "_id": 0})
...
def get_rcb_players():
    # Find the document where IPL2024's key is "Chennai Super Kings"
    rcb_players = db.images.find_one({}, {"IPL 2024.Royal Challengers Bengaluru": 1, "_id": 0})
...
def get_srh_players():
    # Find the document where IPL2024's key is "Chennai Super Kings"
    srh_players = db.images.find_one({}, {"IPL 2024.Sunrisers Hyderabad": 1, "_id": 0})
...
def addMatch():
    ...
    player_exists = db.images.find_one(
...
def get_teams():
    response = list(db.players.aggregate([
              {"$match":{"year":2024}},
              {
                "$group": {
                  "_id": "$team",
...
def get_players_details():
    ...
    response = list(db['test1'].find({"player_name": player_name}))
    response_list = []
    final = {
